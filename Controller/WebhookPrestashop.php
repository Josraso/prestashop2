        ...
        try {
            ...
            if (isset($orderXml->associations->order_histories->order_history)) {
                ...
                if (!empty($historyArray)) {
                    ...
                    Tools::log()->info("Webhook - Fecha último estado (XML associations): {$historyArray[0]}");
            ...
            if (!empty($history)) {
                ...
                if (!empty($dateAdd)) {
                    Tools::log()->info("Webhook - Fecha último estado (API order_histories): {$dateAdd}");
                    return $dateAdd;
                }
            }

            Tools::log()->warning("Webhook - Pedido {$orderId}: no se encontró fecha de último estado en ninguna fuente");
            return null;
        } catch (\Exception $e) {
